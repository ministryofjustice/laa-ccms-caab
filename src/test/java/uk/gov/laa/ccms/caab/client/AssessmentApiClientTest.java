...
class AssessmentApiClientTest {
  ...
  @Mock private WebClient assessmentApiWebClient;
  ...
  @Mock private AssessmentApiClientErrorHandler apiClientErrorHandler;

  @InjectMocks private AssessmentApiClient assessmentApiClient;
  ...
  void getAssessments_success() {
    ...
    when(responseMock.bodyToMono(AssessmentDetails.class))
        .thenReturn(Mono.just(mockAssessmentDetails));

    final Mono<AssessmentDetails> result =
        assessmentApiClient.getAssessments(
            List.of(assessmentName), providerId, caseReferenceNumber);

    StepVerifier.create(result).expectNext(mockAssessmentDetails).verifyComplete();
    ...
    assertEquals(
        "/assessments?name=meansAssessment&provider-id=987&case-reference-number=case456",
        actualUri.toString());
  }
  ...
  @Test
  @DisplayName(
      "updateAssessment succeeds when called with valid assessmentId, assessment, and loginId")
  void updateAssessment_success() {
    ...
    when(requestBodyUriMock.uri("/assessments/{assessment-id}", assessmentId))
    ...
    final Mono<Void> result =
        assessmentApiClient.updateAssessment(assessmentId, assessmentDetail, userLoginId);
  ...
  void patchAssessment_success() {
    ...
    final PatchAssessmentDetail patchDetails =
        new PatchAssessmentDetail(); // Populate this as needed for the test
    ...
    final Mono<Void> result =
        assessmentApiClient.patchAssessment(assessmentId, userLoginId, patchDetails);
  ...
  void deleteAssessments_success() {
    ...
    final ArgumentCaptor<Function<UriBuilder, URI>> uriCaptor =
        ArgumentCaptor.forClass(Function.class);
    when(requestHeadersUriMock.uri(uriCaptor.capture())).thenReturn(requestHeadersMock);
    when(requestHeadersMock.header("Caab-User-Login-Id", userLoginId))
        .thenReturn(requestHeadersMock);
    when(requestHeadersMock.retrieve()).thenReturn(responseMock);
    when(responseMock.bodyToMono(Void.class)).thenReturn(Mono.empty());

    final Mono<Void> result =
        assessmentApiClient.deleteAssessments(
    ...
    assertEquals(
        "/assessments?name=meansAssessment,meritsAssessment&provider-id=987&case-reference-number=case456&status=PENDING",
        actualUri.toString());
  }
}
