package uk.gov.laa.ccms.caab.bean;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.CsvSource;
import org.springframework.test.context.junit.jupiter.SpringExtension;

import static org.junit.jupiter.api.Assertions.*;

@ExtendWith(SpringExtension.class)
public class ClientSearchDetailsTest {

    private ClientSearchDetails clientSearchDetails;

    @BeforeEach
    public void setUp() {
        clientSearchDetails = new ClientSearchDetails();
    }

    @Test
    public void testGetDateOfBirth() {
        // Set the date components
        clientSearchDetails.setDobDay("01");
        clientSearchDetails.setDobMonth("02");
        clientSearchDetails.setDobYear("1990");

        // Get the formatted date of birth
        String dateOfBirth = clientSearchDetails.getDateOfBirth();

        // Check the result
        assertEquals("1990-02-01", dateOfBirth);
    }

    @ParameterizedTest
    @CsvSource({",01,2000",
                "01,,2000",
                "01,01,"})
    public void testGetDateOfBirth_NullComponent(String dobDay, String dobMonth, String dobYear) {
        clientSearchDetails.setDobDay(dobDay);
        clientSearchDetails.setDobMonth(dobMonth);
        clientSearchDetails.setDobYear(dobYear);

        String dateOfBirth = clientSearchDetails.getDateOfBirth();

        assertNull(dateOfBirth);
    }

    @Test
    public void testGetUniqueIdentifier_MatchingType() {
        // Set the unique identifier type and value
        clientSearchDetails.setUniqueIdentifierType(1); // Assuming 1 represents the matching type
        clientSearchDetails.setUniqueIdentifierValue("ABC123");

        // Get the unique identifier
        String uniqueIdentifier = clientSearchDetails.getUniqueIdentifier(1); // Passing matching type 1

        // Check the result
        assertEquals("ABC123", uniqueIdentifier);
    }

    @Test
    public void testGetUniqueIdentifier_NonMatchingType() {
        // Set the unique identifier type and value
        clientSearchDetails.setUniqueIdentifierType(1); // Assuming 1 represents the matching type
        clientSearchDetails.setUniqueIdentifierValue("ABC123");

        // Get the unique identifier with a different matching type
        String uniqueIdentifier = clientSearchDetails.getUniqueIdentifier(2); // Passing a non-matching type 2

        // Check the result, it should be null as the matching type doesn't match
        assertNull(uniqueIdentifier);
    }
}