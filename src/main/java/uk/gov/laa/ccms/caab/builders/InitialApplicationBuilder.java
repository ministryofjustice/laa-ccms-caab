...
public class InitialApplicationBuilder {
   ...
   */
  public InitialApplicationBuilder applicationType(final ApplicationType applicationType) {
  ...
  public InitialApplicationBuilder caseReference(final CaseReferenceSummary caseReferenceSummary) {
    final String caseReference =
        Optional.ofNullable(caseReferenceSummary.getCaseReferenceNumber())
            .orElseThrow(
                () ->
  ...
  public InitialApplicationBuilder provider(final UserDetail user) {
    final IntDisplayValue provider =
        new IntDisplayValue()
  ...
  public InitialApplicationBuilder categoryOfLaw(
      final String categoryOfLawId, final CategoryOfLawLookupValueDetail categoryOfLawLookup) {
    final String categoryOfLawDisplayValue =
        Optional.ofNullable(categoryOfLawLookup)
            .map(CategoryOfLawLookupValueDetail::getMatterTypeDescription)
            .orElse(categoryOfLawId);

    final StringDisplayValue categoryOfLaw =
        new StringDisplayValue().id(categoryOfLawId).displayValue(categoryOfLawDisplayValue);
  ...
  public InitialApplicationBuilder office(final Integer officeId, final List<BaseOffice> offices) {
    final String officeDisplayValue =
        offices.stream()
    ...
    final IntDisplayValue office =
        new IntDisplayValue().id(officeId).displayValue(officeDisplayValue);
  ...
  public InitialApplicationBuilder contractualDevolvedPower(
      final List<ContractDetail> contractDetails, final String categoryOfLawId) {

    final String contractualDevolvedPower =
  ...
  public InitialApplicationBuilder larScopeFlag(final AmendmentTypeLookupDetail amendmentTypes) {
    ...
    final String defaultLarScopeFlag =
        Optional.ofNullable(amendmentTypes.getContent())
            .filter(content -> !content.isEmpty())
            .map(List::getFirst)
            .map(AmendmentTypeLookupValueDetail::getDefaultLarScopeFlag)
            .orElseThrow(
                () -> new RuntimeException("No amendment type available, unable to continue"));
  ...
  public InitialApplicationBuilder status() {
    final StringDisplayValue status =
  ...
  public InitialApplicationBuilder correspondenceAddress() {
    application.correspondenceAddress(new AddressDetail().noFixedAbode(false));
    return this;
  }
