...
public class OrganisationOpponentValidator extends AbstractOpponentValidator {
  ...
  /**
   * Validate the 'other information' field.
   * The field will be restricted to the standard character set.
   * Additionally it must have no more than 2000 characters.
   *
   * @param otherInformationValue - the otherInformation field value.
   * @param errors - the errors.
   */
  private void validateOtherInformation(
      final String otherInformationValue,
      final Errors errors) {
    if (StringUtils.hasText(otherInformationValue)) {
      validateFieldFormat("otherInformation", otherInformationValue,
          STANDARD_CHARACTER_SET, "Other information", errors);
      validateFieldMaxLength("otherInformation", otherInformationValue,
          2000, "Other information", errors);
    }
  }

  /**
   * Validate the 'organisation name' field.
   * This field is mandatory, and must be resctricted to character set A.
   *
   * @param organisationNameValue - the value of the organisationName field.
   * @param errors - the errors
   */
  private void validateOrganisationName(
      final String organisationNameValue,
      final Errors errors) {
    validateRequiredField("organisationName", organisationNameValue,
        "Organisation name", errors);

    if (StringUtils.hasText(organisationNameValue)) {
      validateFieldFormat("organisationName", organisationNameValue,
          CHARACTER_SET_A, "Organisation name", errors);
    }
  }

  /**
   * Validate the 'contact name role' field.
   * This field must be restricted to the standard character set.
   *
   * @param contactNameRoleValue - the value of the form field.
   * @param errors - the errors.
   */
  protected void validateContactNameRole(
      final String contactNameRoleValue,
      final Errors errors) {
    if (StringUtils.hasText(contactNameRoleValue)) {
      validateFieldFormat("contactNameRole",
          contactNameRoleValue, STANDARD_CHARACTER_SET,
